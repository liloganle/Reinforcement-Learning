...
class TDLambda(RandomWalk):
    def __init__(self, alpha, var_lambda):
        ...
        self.old_value = 0              # record the old value
    ...
    def true_online_td(self, state, next_state, reward):
        """Temporal Difference lambda algorithm"""
        # the discounted factor default is 1
        current_state_value = self.get_state_value(state)       # the current state value
        next_state_value = self.get_state_value(next_state)     # the next state value
        delta = reward + next_state_value - current_state_value
        ...
        self.eligibility_trace[state] += 1 - self.alpha * self.var_lambda * self.eligibility_trace[state]
        # update the state value,that is weight vector
        self.weight += self.alpha * (delta + current_state_value - self.old_value) * self.eligibility_trace
        self.weight[state] -= self.alpha * (current_state_value - self.old_value)
        self.old_value = current_state_value
...
def solve_method(method_class):
    ...
    while init_state not in method_class.end_state:
        ...
        method_class.true_online_td(init_state, next_state, reward_temp)
...
if __name__ == "__main__":
    ...
    plt.title(r"True Online TD($\lambda$) algorithm")
    plt.legend()
    plt.savefig("./images/Figure12-8.png")
    plt.show()
    plt.close()
