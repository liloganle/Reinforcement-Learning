...
class DynaMaze(object):
    def __init__(self, world_size=(6, 9), obstacle=[[0, 7], [1, 2], [1, 7], [2, 2], [2, 7], [3, 2], [4, 5]],
        ...
        self.cur_obstacle = [[3, row] for row in range(world_size[1] - 1)]
        self.new_obstacle = [[3, row] for row in range(1, world_size[1])]
...
class EnhancedModel(object):
    ...
    def building(self, state, action, next_state, reward):
        ...
        if tuple(state) not in self.model.keys():
            self.model[tuple(state)] = {}
            # action that have never been selected before should been considered
...
def cumulative_reward(runs, methods):
    dyna_maze = DynaMaze(planning_steps=10)
...
if __name__ == "__main__":
    ...
    cumulative_rewards = cumulative_reward(runs=num_runs, methods=two_models)
    ...
    plt.legend()
    plt.savefig("./images/Figure8-4.png")
    plt.show()
    plt.show()
    plt.close()
